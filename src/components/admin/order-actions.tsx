...
export function AdminOrderActions({ order }: { order: any }) {
  ...
  return (
    <>
      ...
      {canMarkPaid && (
        <Button variant="outline" size="sm" onClick={() => handle('paid')} title={t('admin.orders.markPaid')}>
          <CheckCircle className="h-3.5 w-3.5" />
        </Button>
      )}
      {canMarkDelivered && (
        <Button variant="outline" size="sm" onClick={() => handle('delivered')} title={t('admin.orders.markDelivered')}>
          <Truck className="h-3.5 w-3.5" />
        </Button>
      )}
      {canCancel && (
        <Button variant="destructive" size="sm" onClick={() => handle('cancel')} title={t('admin.orders.cancel')}>
